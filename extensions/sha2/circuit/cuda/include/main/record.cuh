...
template <typename V> struct Sha2RecordHeader {
    uint32_t variant;
    uint32_t from_pc;
    uint32_t timestamp;
...
template <typename V> struct Sha2RecordMut {
    ...
    __device__ __host__ __forceinline__ Sha2RecordMut(uint8_t *record_buf) {
        header = reinterpret_cast<Sha2RecordHeader<V> *>(record_buf);
        uint32_t offset = sizeof(Sha2RecordHeader<V>);
