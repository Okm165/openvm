...
template <typename T> struct DeviceBufferConstView {
    T const *ptr;
    size_t size;

    __device__ __host__ __forceinline__ T const *begin() const { return ptr; }

    __device__ __host__ __forceinline__ T const *end() const { return ptr + len(); }
...
struct DeviceRawBufferConstView {
    ...
    template <typename T> __device__ __host__ DeviceBufferConstView<T> as_typed() const {
        ...
        return {reinterpret_cast<T const *>(ptr), size};
    }
};
